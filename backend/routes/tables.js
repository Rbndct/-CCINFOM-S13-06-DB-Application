  ...
  try {
    ...
    if (exists.length > 0) {
      return res.status(409).json(
          {success: false, error: 'Couple table already exists'});
    }
  ...
  try {
    ...
    if (!Number.isInteger(cap) || cap < 6 || cap > 10) {
      return res.status(400).json({
        success: false,
        error: 'Capacity must be an integer between 6 and 10'
      });
    }
    ...
    res.status(201).json({
      success: true,
      message: 'Guest table created',
      data: {
        table_id: ins.insertId,
        wedding_id: Number(weddingId),
        table_number: nextNum,
        table_category: 'guest',
        capacity: cap
      }
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to create guest table',
      message: error.message
    });
  }
});
...
router.delete('/seating/:table_id', async (req, res) => {
  try {
    ...
    if (rows[0].table_category === 'couple') {
      return res.status(400).json(
          {success: false, error: 'Couple table cannot be deleted'});
    }
    const [g] = await promisePool.query(
        'SELECT COUNT(*) AS cnt FROM guest WHERE table_id = ?', [tableId]);
    if ((g[0]?.cnt || 0) > 0) {
      return res.status(409).json(
          {success: false, error: 'Cannot delete table with assigned guests'});
    }
    await promisePool.query(
