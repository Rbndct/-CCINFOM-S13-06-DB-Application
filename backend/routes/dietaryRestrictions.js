...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch dietary restrictions', message: error.message });
  }
});
...
router.get('/:restriction_id', async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ success: false, error: 'Dietary restriction not found' });
    }
    res.json({ success: true, data: rows[0] });
  } catch (error) {
    console.error('Error fetching dietary restriction:', error);
    res.status(500).json({ success: false, error: 'Failed to fetch dietary restriction', message: error.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!restriction_name || !severity_level || !restriction_type) {
      return res.status(400).json({ success: false, error: 'Missing required fields: restriction_name, severity_level, restriction_type' });
    }
    ...
    res.status(201).json({ success: true, message: 'Dietary restriction created', data: { restriction_id: ins.insertId, restriction_name, severity_level, restriction_type } });
  } catch (error) {
    console.error('Error creating dietary restriction:', error);
    res.status(500).json({ success: false, error: 'Failed to create dietary restriction', message: error.message });
  }
});
...
router.put('/:restriction_id', async (req, res) => {
  try {
    ...
    const [upd] = await promisePool.query(
      ...
      [restriction_name ?? null, severity_level ?? null, restriction_type ?? null, req.params.restriction_id]
    );
    ...
    res.status(500).json({ success: false, error: 'Failed to update dietary restriction', message: error.message });
  }
});
...
router.delete('/:restriction_id', async (req, res) => {
  try {
    ...
    if ((guest_refs || 0) > 0 || (pref_refs || 0) > 0 || (menu_refs || 0) > 0) {
      return res.status(409).json({ success: false, error: 'Restriction in use and cannot be deleted' });
    }
    ...
    res.status(500).json({ success: false, error: 'Failed to delete dietary restriction', message: error.message });
  }
});
